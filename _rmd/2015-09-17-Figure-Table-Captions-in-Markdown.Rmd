...
```{r ExHistL_Captioner, fig.cap=figs("LenFreq1")}
...
As more functions are added, the figure numbers are incremented such that inline R code may refer to `r figs("WtFreq1",display="cite")` and `r figs("LenFreq1",display="cite")` and the `fig.cap` chunk option may still be used to add the caption below the figure.

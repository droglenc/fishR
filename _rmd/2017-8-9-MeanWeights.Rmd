...
```{r Sum1Tab, echo=FALSE, results="asis"}
print(xtable(sum1,digits=c(0,0,0,0,0,0,2)),type="html",include.rownames=FALSE,comment=FALSE)
```
...
```{r Sum2Tab, echo=FALSE, results="asis"}
sum2 <- group_by(df,age) %>%
  summarize(n=n(),mnlen=mean(len),true.mnwt=mean(wt2)) %>%
  mutate(pred.mnwt=exp(loga+b*log(mnlen)),
         diff.mnwt=(pred.mnwt-true.mnwt)/true.mnwt*100) %>%
  as.data.frame()
print(xtable(sum2,digits=c(0,0,0,0,0,0,2)),type="html",include.rownames=FALSE,comment=FALSE)
```
...
```{r Sum3Tab, echo=FALSE, results="asis"}
b <- 2.5
lens <- NULL
for (i in 1:length(ages)) lens <- c(lens,rnorm(ns[i],mean=mns[i],sd=sds[i]))
df <- data.frame(age=rep(ages,ns),len=round(lens,0)) %>%
...
sum3 <- group_by(df,age) %>%
  ...
  mutate(pred.mnwt=exp(loga+b*log(mnlen)),
         diff.mnwt=(pred.mnwt-true.mnwt)/true.mnwt*100) %>%
  as.data.frame()
print(xtable(sum3,digits=c(0,0,0,0,1,1,2)),type="html",include.rownames=FALSE,comment=FALSE)
```

`r tabs("Sum4Tab")`

```{r Sum4Tab, echo=FALSE, results="asis"}
b <- 3.0
lens <- NULL
for (i in 1:length(ages)) lens <- c(lens,rnorm(ns[i],mean=mns[i],sd=sds[i]))
df <- data.frame(age=rep(ages,ns),len=round(lens,0)) %>%
...
sum4 <- group_by(df,age) %>%
  ...
  mutate(pred.mnwt=exp(loga+b*log(mnlen)),
         diff.mnwt=(pred.mnwt-true.mnwt)/true.mnwt*100) %>%
  as.data.frame()
print(xtable(sum4,digits=c(0,0,0,0,0,0,2)),type="html",include.rownames=FALSE,comment=FALSE)
```

`r tabs("Sum5Tab")`

```{r Sum5Tab, echo=FALSE, results="asis"}
b <- 3.5
lens <- NULL
for (i in 1:length(ages)) lens <- c(lens,rnorm(ns[i],mean=mns[i],sd=sds[i]))
df <- data.frame(age=rep(ages,ns),len=round(lens,0)) %>%
...
sum5 <- group_by(df,age) %>%
  ...
  mutate(pred.mnwt=exp(loga+b*log(mnlen)),
         diff.mnwt=(pred.mnwt-true.mnwt)/true.mnwt*100) %>%
  as.data.frame()
print(xtable(sum5,digits=c(0,0,0,0,0,0,2)),type="html",include.rownames=FALSE,comment=FALSE)
```
